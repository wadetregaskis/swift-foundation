        ...
        try auxPath.withFileSystemRepresentation { auxPathFileSystemRep in
            ...
            try newPath.withFileSystemRepresentation { newPathFileSystemRep in
                ...
                if options.contains(.withoutOverwriting) {
                    // To get atomic write + fail-on-exists in one operation, hard-link
                    // the aux file at the destination and then unlink the aux name.
                    // link(2) atomically fails with EEXIST if the destination already
                    // exists, which is the contract of `.withoutOverwriting`.
